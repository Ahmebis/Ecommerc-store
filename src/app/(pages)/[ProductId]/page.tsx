...
export default async function ProductDetails({ params }: { params: Params }) {
  ...
  const allProductsRes = await fetch(
    "https://ecommerce.routemisr.com/api/v1/products?fields=id,title"
  );
  const allProductsData = await allProductsRes.json();
  const allProducts: ProductI[] = allProductsData.data;

  const currentProductIndex = allProducts.findIndex((n) => n.id === ProductId);
  ...
  return (
    <>
      ...
      <div className="flex justify-between mt-8 px-4 max-w-4xl mx-auto">
        {prevProduct ? (
          <Button asChild variant="outline">
            <a href={`/${prevProduct.id}`}> Previous</a>
          </Button>
        ) : (
          <div />
        )}
